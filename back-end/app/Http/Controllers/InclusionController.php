...
class InclusionController extends Controller
{
    ...
    public function getAll(){
        ...
        {
            $data = Inclusion::where('description', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
            $data = Inclusion::all();
        }
    ...
    public function save(){
        ...
        $result = Inclusion::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response(Inclusion::whereId($result->id)->first());
    }
    public function delete(){
        if(!$this->role_permission_svc->inclusionDelete())
            return response('Authorization Required',401);
        $data = $this->request->all();
        $result = Inclusion::find( $data['id'] );
