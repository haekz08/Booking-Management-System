...
use App\IdentificationCardType;
...
class IdentificationCardTypeController extends Controller
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->model = new IdentificationCardType();
    ...
    public function getAll(){
        ...
        {
            $data = IdentificationCardType::where('name', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
            $data = IdentificationCardType::all();
        }
    ...
    public function save(){
        ...
        $result = IdentificationCardType::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response(IdentificationCardType::whereId($result->id)->first());
    }
    public function delete(){
        if(!$this->role_permission_svc->superAdminRestriction())
            return response('Authorization Required',401);
        $data = $this->request->all();
        $result = IdentificationCardType::find( $data['id'] );
