...
class NationalityController extends Controller
{
    ...
    public function getAll(){
        ...
        {
            $data = Nationality::where('description', 'like', '%'.$this->search.'%')->orderByRaw('is_preferred DESC, description')->paginate($this->per_page);
        }else
        {
    ...
    public function save(){
        ...
        $result = Nationality::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response(Nationality::whereId($result->id)->first());
    }
    public function delete(){
        if(!$this->role_permission_svc->superAdminRestriction())
            return response('Authorization Required',401);
        $data = $this->request->all();
        $result = Nationality::find( $data['id'] );
