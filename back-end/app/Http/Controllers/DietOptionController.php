...
class DietOptionController extends Controller
{
    ...
    public function getAll(){
        ...
        {
            $data = DietOption::where('description', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
            $data = DietOption::all();
        }
    ...
    public function save(){
        ...
        $result = DietOption::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response(DietOption::whereId($result->id)->first());
    }

    public function delete(){
        $data = $this->request->all();
        $result = DietOption::find( $data['id'] );
