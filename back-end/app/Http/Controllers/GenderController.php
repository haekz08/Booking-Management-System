...
class GenderController extends Controller
{
    ...
    public function getAll(){
        ...
        {
            $data = Gender::where('description', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
            $data = Gender::all();
        }
    ...
    public function save(){
        ...
        $result = Gender::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response(Gender::whereId($result->id)->first());
    }

    public function delete(){
        $data = $this->request->all();
        $result = Gender::find( $data['id'] );
