...
class CrewTypeController extends Controller
{
    ...
    public function getAll(){
        ...
        {
            $data = CrewType::where('description', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
            $data = CrewType::all();
        }
    ...
    public function save(){
        ...
        $result = CrewType::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response(CrewType::whereId($result->id)->first());
    }
