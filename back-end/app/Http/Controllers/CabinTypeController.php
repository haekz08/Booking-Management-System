...
class CabinTypeController extends Controller
{
    ...
    public function getAll(){
        if(!$this->role_permission_svc->cabinTypeRead())
            return response('Authorization Required',401);

        if(isset($this->request->per_page))
        {
            $data = CabinType::where('name', 'like', '%'.$this->search.'%')->paginate($this->per_page);
        }else
        {
            $data = CabinType::all();
        }
    ...
    public function save(){
        ...
        if (isset($data['id']) && $data['id'] != -1) {
            ...
            if(!$this->role_permission_svc->cabinTypeUpdate())
                return response('Authorization Required',401);
        } else {
            $data['created_by'] = $this->getUserId();
            $data['updated_by'] = $this->getUserId();
            if(!$this->role_permission_svc->cabinTypeAdd())
                return response('Authorization Required',401);
        }
        $result = CabinType::updateOrCreate(['id'=>$data['id']],$data);
        return empty($result) ? response('Internal Server Error',500) : response(CabinType::whereId($result->id)->first());
    }
    public function delete(){
        if(!$this->role_permission_svc->cabinTypeDelete())
            return response('Authorization Required',401);
        $data = $this->request->all();
        $result = CabinType::find( $data['id'] );
