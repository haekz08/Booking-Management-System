        ...
        <b-modal header-text-variant="light" header-bg-variant="dark" footer-bg-variant="dark" centered
            ...
            <div class="custom-modal-body">
                <b-card class="mb-0">
                    ...
                    <b-form-group
                            label="Make this country preferred"
                            label-for="is_preferred"
                            class="mb-0"
                    >
                        <label class="switch switch-pill switch-label switch-success">
                            <input type="checkbox" class="switch-input" id="is_preferred" v-model="form_data.is_preferred">
                ...
                this.swalConfirmSubmit((data)=>{
                    if (data.value) {
                        this.$validator.validateAll().then(result=>{
                            if(result){
                                this.is_saving=true;
                                this.axios.post('maintenance/nationalities/save',this.form_data).then(response => {
                                    this.closeModal();
