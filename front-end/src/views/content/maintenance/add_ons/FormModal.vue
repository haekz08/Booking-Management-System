        ...
        <b-modal header-text-variant="light" header-bg-variant="dark" footer-bg-variant="dark" centered
            ...
            <div class="custom-modal-body">
                <b-card class="mb-0">
                    ...
                    <b-form-group
                            :description="errors.has('price') ? '' : '*This field is required.'"
                            :invalid-feedback="!errors.has('price') ? '' : '*'+errors.first('price')"
                            :state="false"
                            label="Price"
                            label-for="price"
                            class="mb-4"
                    >
                        <input type="number" step="any" class="form-control" :state="true" v-model="form_data.price" v-validate="'required|decimal:2|min_value:1'" id="price" name="price" v-bind:class="{ 'is-invalid': errors.has('price') }">
                    </b-form-group>
                    ...
                    <b-form-group
                            label="Offered as default to all itineraries?"
                            label-for="is_default_offered"
                            class="mb-0"
                    >
                        <label class="switch switch-pill switch-label switch-success">
                            <input type="checkbox" class="switch-input" id="is_default_offered" v-model="form_data.is_default_offered">
                ...
                this.swalConfirmSubmit((data)=>{
                    ...
                    if (data.value) {
                        this.$validator.validateAll().then(result=>{
                            if(result){
                                this.is_saving=true;
                                this.axios.post('maintenance/add_ons/save',this.form_data).then(response => {
                                    this.closeModal();
