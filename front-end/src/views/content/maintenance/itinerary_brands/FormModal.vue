            ...
            <div class="custom-modal-body">
                <b-card class="mb-0">
                    ...
                    <b-form-group
                            ...
                            :state="false"
                            label="Itinerary Brand Slug Name"
                            label-for="slug"
                            class="mb-4"
                    >
                    ...
                    <b-form-group
                            :description="errors.has('color') ? '' : '*This field is required.'"
                            :invalid-feedback="!errors.has('color') ? '' : '*'+errors.first('color')"
                            :state="false"
                            label="Brand Color"
                            label-for="color"
                            class="mb-4"
                    >
                        <input type="text" class="form-control" v-model="form_data.color" v-validate="'required'" id="color" name="color" v-bind:class="{ 'is-invalid': errors.has('color') }">
                    </b-form-group>
                    ...
                    <b-form-group
                            label="Enable ambassador to this brand?"
                            label-for="is_ambassador_required"
                            class="mb-0"
                    >
                        <label class="switch switch-pill switch-label switch-success">
                            <input type="checkbox" class="switch-input" id="is_ambassador_required" v-model="form_data.is_ambassador_required">
                ...
                this.swalConfirmSubmit((data)=>{
                    ...
                    if (data.value) {
                        this.$validator.validateAll().then(result=>{
                            if(result){
                                this.is_saving=true;
                                this.axios.post('maintenance/itinerary_brands/save',this.form_data).then(response => {
                                    this.closeModal();
